...
const SEP='[\\s\\p{P}\\p{S}\\u200B-\\u200F\\u202A-\\u202E\\u2060-\\u2064\\uFEFF]*';
const CONF={
  a:'[aAаɑ]',b:'[bB]',e:'[eEе]',g:'[gG]',i:'[iIіІıɩ]',j:'[jJ]',l:'[lLⅼӏ]',m:'[mM]',n:'[nN]',o:'[oOοо0]',q:'[qQ]',r:'[rR]',s:'[sS]',u:'[uU]',w:'[wW]',y:'[yY]'
};
...
function collectTokens(line){
  const out=[];
  for(const d of tokenDefs){
    d.rx.lastIndex=0;
    for(const m of line.matchAll(d.rx)){
      if(!m[0])continue;
      out.push({kind:d.kind,name:d.name,start:m.index,end:m.index+m[0].length});
    }
  }
...
function clusters(tokens){
  if(!tokens.length)return[];
  const out=[];let cur=[tokens[0]];
  for(const t of tokens.slice(1)){
    const prev=cur[cur.length-1];
    if(t.start-prev.end<=56)cur.push(t);else{out.push(cur);cur=[t]}
  }
  out.push(cur);return out;
}
function extend(line,a,b){
  const noisy=c=>/[\s\p{P}\p{S}0-9０-９]/u.test(c)||STRONG.has(c)||ROMAN.test(c)||CIRCLED.test(c);
  let x=a,y=b;
  while(x>0&&a-x<18&&noisy(line[x-1]))x--;
  while(y<line.length&&y-b<28&&noisy(line[y]))y++;
  return[x,y];
}
function scoreCluster(line,ts,a,b){
  const core=line.slice(a,b),familyTokens=ts.filter(x=>x.kind==='family'),family=familyTokens.length>0,qun=ts.some(x=>x.kind==='qun'),num=ts.filter(x=>x.kind==='num'&&!familyTokens.some(f=>x.start>=f.start&&x.end<=f.end)).length;
  const digit=digitCount(core),special=specialCount(core),strong=strongCount(core),punct=punctCount(core);
  let score=0;
  if(family)score+=6;
  if(qun)score+=5;
  if(num>=2)score+=4;
  if(num>=4)score+=3;
  if(num>=1&&(digit+special+strong)>=2)score+=3;
  if(num>=1&&special>=2)score+=3;
  if(special)score+=2;
  if(strong>=2)score+=3;
  if(punct>=4)score+=1.5;
  if(family&&(qun||num||digit||special||strong))score+=2;
  if(STORY_GROUP.test(line)&&!family&&num<2&&digit===0&&special===0&&strong===0)score-=7;
  if(DOMAIN.test(line)&&!family&&!qun)score-=5;
...
export function detectAdPinyin(text){
  ...
  lines.forEach((line,i)=>{
    const ts=collectTokens(line);
    for(const c of clusters(ts)){
      let a=Math.min(...c.map(x=>x.start)),b=Math.max(...c.map(x=>x.end));
      [a,b]=extend(line,a,b);
      const e=scoreCluster(line,c,a,b);
      const high=(e.family&&(e.qun||e.num>=1||e.digit+e.special+e.strong>=2))||(e.qun&&e.num>=2)||(e.num>=3&&(e.special+e.strong+e.digit>=1))||(e.num>=1&&e.special+e.strong>=3);
      const review=(e.family||(e.qun&&(e.num>=1||e.special+e.strong>=2))||e.num>=2||(e.num>=1&&e.special>=2))&&e.score>=5;
      if(!high&&!review)continue;
      out.push(matchObj(i+1,line,a,b,e.score,high&&e.score>=8?'delete':'review',`拼音化广告优先检测：${e.family?'广告主体 ':''}${e.qun?'Qun/群 ':''}${e.num?`数字拼音×${e.num} `:''}${e.special?'圈号/罗马数字 ':''}${e.strong?'乱码混写':''}`.trim()));
    }
  });
  return out;
}
export function mergeAdPinyinScan(result,text,applyMatches,preserveBlank=true){
  ...
  for(const m of detectAdPinyin(text)){
    const overlap=existing.some(x=>x.line===m.line&&!(x.end<=m.start||m.end<=x.start));
    if(overlap)continue;
    (m.action==='delete'?auto:review).push(m);existing.push(m);
  }
...
export function gatePinyinResult(text,result){
  ...
  for(const m of detectAdPinyin(text))if(m.score>=5)suspicious.set(m.line,m.reason);
  if(!suspicious.size)return result;
  const original=String(text||'').split(/\r?\n/),produced=String(result?.text||text).split(/\r?\n/);
  for(const [line,reason] of suspicious){if(line>=1&&line<=produced.length)produced[line-1]=original[line-1];}
  const auto=(result?.auto||[]).filter(x=>!suspicious.has(x.line));
  const review=(result?.review||[]).filter(x=>!suspicious.has(x.line));
